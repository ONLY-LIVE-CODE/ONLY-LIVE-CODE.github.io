...
class goods extends Admin_Controller {
	function __Construct() {
		...
		if ($this->session->userdata('id') != 45) redirect('/admin/stat');
    }
	...
	public function edit ($id = NULL)
	{
		...
		if($this->form_validation->run() == TRUE) {
			$data = $this->goods_model->array_from_post(array('name','category','descr','iconurl','price_rub','price_dlr','type_Item','skidka','min_order','sell_method','goods','onmain','janr','yazuk','platforma','mylytplayeer','relyz','izdatel','atkiv','foto1','foto2','foto3','foto4'));
			...
			if ($data['sell_method'] == 1) {
				...
				{
					$data = $this->goods_model->array_from_post(array('name','descr','iconurl','price_rub','price_dlr','type_Item','skidka','min_order','janr','yazuk','platforma','mylytplayeer','relyz','izdatel','atkiv'));
