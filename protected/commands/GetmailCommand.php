                        ...
                        if ($ban['ban'] !== true) {
                            ...
                            if ($result !== null && $result['bool'] === true) {
                                ...
                                if ($message['plain'] !== '' AND empty($message['html'])) {
                                    ...
                                    if ($configuration['getmaildisableconvert'] == 1) {
                                        ...
                                        try {
                                            $decoded = mb_convert_encoding($message['html'], 'UTF-8',
                                                $message['charset'] ? $message['charset'] : 'auto');
                                            $html = new \Html2Text\Html2Text($decoded);
                                            $pre = $html->getText();
                                        } catch (Exception $e) {
                                            $msg = $e->getMessage();
                                ...
                                if(isset($template)) {
                                    ...
                                    $ucomments = Comments::model()->findAllByAttributes(array('rid' => $result['request'], 'show' => 0));
                                    arsort($ucomments);
                                    $uctext = null;
                                ...
                                if (isset($ticket->tchat_id) AND !empty($ticket->tchat_id) AND Yii::app()->params['TBotEnabled'] == 1) {
                                    ...
                                    $telegram->sendMessage([
                                        ...
                                        'parse_mode' => 'HTML',
                                        'text' => strip_tags($mdl->comment, '<i><a><b><code><pre><strong><em>')
                                    ]);
                                    if (isset($afiles)) {
                                ...
                                if (isset($usermail) AND $usermail->sendmail == 1 AND $usermail !== $managermail) {
                                    SendMail::send($uaddress, $ssubject, $umessage, $afiles);
                                }
                                ...
                                if (isset($result) && $result['bool'] == false) {
                                    ...
                                    $subj = preg_replace('/^((Re|На|От|Fwd?)(\[\d+\])?: |(Re|На|От|Fwd?)(\(\d+\))?: )+/iu', '', $message['subject']);
                                ...
                                $model->Name = $subject;
                                $model->channel = 'Email';
                                $model->channel_icon = 'iicon iicon-envelope';
                                if (isset($service)) {
                                    $model->Priority = $service['priority'];
                                    $watchers = explode(',', $service['watcher']);
                                    $watchers = array_values(array_diff($watchers, array("null","")));
                                    $model->watchers = $exists ? implode(',', array_unique(array_merge($watchers, $exists))): $service['watcher'];
                                    if ($service['gtype'] == 1) {
                                ...
                                if ($message['plain'] !== '' AND empty($message['html'])) {
                                    ...
                                    $model->Content = ($nlbr == 1) ? $decoded : nl2br($decoded);
                                } else {
                                    if ($configuration['getmaildisableconvert'] == 1) {
                                        ...
                                        $model->Content = ($nlbr == 1) ? $content : nl2br($content);
                                    } else {
                                        try {
                                            $emailContent = $message['html'];
                                        ...
                                        $visible = $email->getVisibleText();
                                        if ($trim == 1) {
                                            $decoded = ($nlbr == 1) ? $emailContent : nl2br($emailContent);
                                        } else {
                                            $decoded = ($nlbr == 1) ? $visible : nl2br($visible);
                                        }
