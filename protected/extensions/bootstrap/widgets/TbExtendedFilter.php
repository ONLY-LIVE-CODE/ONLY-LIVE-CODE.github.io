...
class TbExtendedFilter extends CWidget
{
	...
	public function init()
	{
		...
		$this->registry .= '-' . $this->grid->id . Yii::app()->user->id;

		$this->jsonStorage = new JSONStorage();
		$this->jsonStorage->addRegistry($this->registry);

		$attributes = [];
        if ($this->model instanceof RequestFullFields) {
            $attributes = $this->model->getFullAttributes();
        } else {
	...
	public function registerClientScript()
	{
		...
		Yii::app()->clientScript->registerScript(
			__CLASS__ . '#extended-filter' . $this->grid->id . Yii::app()->user->id,
			<<<EOD
           		$(document).on('click', '#{$this->grid->id} .btn-extended-filter-save', function(e){
			e.preventDefault();
			bootbox.prompt("Введите название для фильтра?", "Отмена", "Сохранить", function(result){
		...
		$(document).on('click', '#{$this->grid->id} .btn-extended-filter-delete', function(e) {
			...
			bootbox.confirm('Удалить фильтр "'+option.text()+'"?', function(confirmed){
				if (confirmed)
				{
...
EOD
	...
	protected function displaySaveButton($registryKey)
	{
		...
		echo CHtml::link(
			'Сохранить фильтр',
			'javascript:void(0)',
			array('class' => 'btn btn-small btn-default btn-extended-filter-save')
		);
	...
	protected function displaySavedFilters($registryKey)
	{
		if ($this->jsonStorage->getLength($this->registry)) {
			...
			echo '<div><span class="span" style="margin-left:0px">';
			...
			echo '<option value="-1" data-filter="{}" ' . (!$registryKey ? 'selected' : '') . '>Без фильтра</option>';
			...
			echo CHtml::link(
				'<i class="iicon iicon-funnel"></i>',
				'javascript:viod(0)',
				array('class' => 'btn btn-extended-filter-apply', 'style' => 'margin-bottom:9px', 'title' => Yii::t('main-ui', 'Apply filter'))
			);

			echo CHtml::link(
				'<i class="iicon iicon-trash2"></i>',
				'javascript:viod(0)',
				array('class' => 'btn btn-extended-filter-delete', 'style' => 'margin-bottom:9px' , 'title' => Yii::t('main-ui', 'Remove filter'))
			);
			if ($this->grid->id == 'request-grid-full' OR $this->grid->id == 'request-grid-full2'){
				if (Yii::app()->session['requestSaveFilter']){
					echo "<a href=\"javascript:void(0);\" onclick=\"$.ajax({data:{savefilter:'clear'}, success: function(){location.reload();}});\" id=\"clean-filter\" class=\"btn\" style = \"margin-bottom:9px\">
									<i class=\"iicon iicon-cross\" title=\"".Yii::t('main-ui', 'Clear filter')."\"></i> </a>";
				} else {
					echo "<a href=\"javascript:void(0);\" onclick=\"$.ajax({data:{savefilter:'save'}, success: function(){location.reload();}});\" id=\"save-filter\"  class=\"btn\" style = \"margin-bottom:9px\">
									<i class=\"iicon iicon-pushpin\" title=\"".Yii::t('main-ui', 'Save filter')."\"></i> </a>";
				}
			}
			echo '</span></div>';
		}
